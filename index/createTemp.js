...
const TOKEN = "5dfb5b2d26fc827bf151d0de994064a0e0a25e55a84384dd3aefda51873e1fe2";
    ...
    .then((response) => {
        ...
        fs.writeFile(filePath, csvData, (err) => {
            if (err) {
                ...
                console.log('CSV file has been saved in CSV folder with name =', path.join(fileName));
                console.log('Filepath =', path.join(folderPath));
            }
        });
    })
    .catch((error) => {
        console.error(`Test case is failed to create`, error.response ? error.response.data : error.message);
    });
...
function convertToCSV(objArray) {
    ...
    const headers = ['v2.id']
        .concat(Object.keys(array[0]).filter(key => key !== 'steps' && key !== 'title'))
    ...
    for (let i = 0; i < array.length; i++) {
        ...
        const baseFields = ['v2.id']
            .concat(Object.keys(row).filter(key => key !== 'steps' && key !== 'title'))
            .map(key => {
                if (key === 'behavior') {
        ...
        for (let j = 0; j < steps.length; j++) {
            const step = steps[j];
            const steps_actions = `${j + 1}. ${step.action ? JSON.stringify(step.action) : ''}`;
            const steps_data = `${j + 1}. ${step.data ? JSON.stringify(step.data) : ''}`;
            const steps_result = `${j + 1}. ${step.expected_result ? JSON.stringify(step.expected_result) : ''}`;
